...
document.addEventListener("DOMContentLoaded", function(event) {
	...
	function start() {
		...
		navigator.mediaDevices.getUserMedia({
			audio: true,
			video: true
		}).then(gotStream).catch(e => console.log(`getUserMedia() error: ${e.name}`));
	}
	...
	function onIceCandidate(pc, event) {
		getOtherPc(pc).addIceCandidate(event.candidate).then(() => onAddIceCandidateSuccess(pc), err => onAddIceCandidateError(pc, err));
