(function(window) {
	...
	p.onKeyUp = function(e) {
		...
		if (e.shiftKey) {
			if (k == 40) this.setSpeed(this.speed == 0 ? cfg.autospeed : 0);
			else if (k == 38) this.setSpeed(this.speed == 0 ? -cfg.autospeed : 0);
			else if (k == 34) this.changeTrail(1.01 * cfg.sheetLength);
			else if (k == 33) this.changeTrail(-1.01 * cfg.sheetLength);
			else if (k == 36) this.setTrail(cfg.trailMin);
			else if (k == 35) this.setTrail(cfg.rollLength - cfg.sheetLength);
			else if (k == 12) this.setSpeed(0);
		} else if (k == 40) this.changeTrail(cfg.keyspeed);
		else if (k == 38) this.changeTrail(-cfg.keyspeed);
	...
	p.start = function() {
		...
		cfg.perimeterMin = cfg.radiusMin * 2 * Math.PI;
	...
	p.drawRoll = function(radius, trail, remainder) {
		...
		var rest = trail % cfg.sheetLength;
		...
		var aBtm = this.getPerspectiveAngle(0, radius) - PI5;
		aBtm += PI2;
		...
		var aOrigin = this.getPerspectiveAngle(0, radius) + 2.5 * PI;
		var curveTopLen = -aTop * radius;
		...
		if (remainder > cfg.perimeterMin) {
			var sheetArc = cfg.sheetLength / radius;
			var firstArc = (cfg.sheetLength - rest) / radius;
		...
		if (remainder < curveTopLen) {
			if (remainder > 0) {
				...
				g.mt(-radius, 0).lt(-radius, trailStraight).lt(px - radius2, py + trailStraight * f).lt(px - radius2, py).arc(px, py, radius2, PI, ang + PI).lt(p.x, p.y).arc(0, 0, radius, ang + PI, PI, true);
			} else {
				g.mt(-radius, -remainder).lt(-radius, trailStraight).lt(px - radius2, py + trailStraight * f).lt(px - radius2, py - remainder * f);
			}
		} else {
			var perspAngle = this.getPerspectiveAngle(0, -radius) - PI5;
			if (perspAngle < -PI) perspAngle += PI2;
			g.mt(-radius, 0).lt(-radius, trailStraight).lt(px - radius2, py + trailStraight * f).lt(px - radius2, py).arc(px, py, radius2, PI, aTop).lt(pTop.x, pTop.y).arc(0, 0, radius, aTop, PI, true);
		}
		g.cp().ef();
